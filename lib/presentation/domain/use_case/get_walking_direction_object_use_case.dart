...
class GetWalkingDirectionObjectUseCase implements FutureUseCase<DirectionObjectInput,DirectionObject>{
  @override
  Future<DirectionObject> run(DirectionObjectInput input) async{
    return await injector.get<UserMapRepository>().getWalkingDirection(input.direction, input.accessToken, input.annotations, input.geometries, input.overviews);
  }
...
class DirectionObjectInput{
  ...
  DirectionObjectInput(this.direction, this.accessToken, this.annotations, this.geometries, this.overviews);
